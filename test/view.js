$(document).ready(function() {
  ...
  test("View: _ensureElement with DOM node el", function() {
    var ViewClass = Colonne.View.extend({
      el: document.body
    });
  ...
  test("View: with attributes", function() {
    var view = new Colonne.View({attributes : {'class': 'one', id: 'two'}});
  ...
  test("View: multiple views per element", function() {
    var count = 0, ViewClass = Colonne.View.extend({
      el: $("body"),
      events: {
        "click": "click"
      },
  ...
  test("#1048 - setElement uses provided object.", function() {
    var $el = $('body');
    var view = new Colonne.View({el: $el});
    ok(view.$el === $el);
  ...
  test("Clone attributes object", function() {
    var View = Colonne.View.extend({attributes: {foo: 'bar'}});
	...
	test("template id is set", function () {
		var Product = Backbone.Model.extend({ 
			defaults: { foo: 'bar' }
		});
		var View = Colonne.View.extend({ model: new Product() });
		var v = new View({ id: 'colonne-product' });
		v.render();
		equal('colonne-product-template', v.templateId);
	});
	...
	test("template is rendered", function () {
		...
		var v = new View({ id: 'colonne-product' });
		v.render();
		ok(v.$el.html().match(/bar/));
	});
	
	test("template is not rendered when id is incorrect", function () {
		var Product = Backbone.Model.extend({ 
			defaults: { foo: 'bar' }
		});
		var View = Colonne.View.extend({ model: new Product() });
		var v = new View({ id: 'colonne-product1' });
		v.render();
		ok(v.$el.html() === '');
	});
	...
	test("template is cached", function () {
		...
		var v = new View({ id: 'colonne-product' });
		v.render();
		ok(Colonne.View.TemplateStore[v.templateId]);
	});	
});
