$(document).ready(function () {
  ...
  test("Backbone.History extended with path and params", function () {
    equal(typeof Backbone.history.path, 'string');
		equal(typeof Backbone.history.params, 'object');
  });
	
  test("Gets the path correctly", function () {
    var route = '/products?name=apple&page=1';
    Backbone.history.navigate(route, {trigger: true});
    equal(Backbone.history.path, 'products');
		window.location.hash = '';
  });

  test("Gets the params correctly", function () {
    var route = '/products?name=apple&page=1';
    Backbone.history.navigate(route, {trigger: true});
    equal(Backbone.history.params['name'], 'apple');
		equal(Backbone.history.params['page'], '1');
		window.location.hash = '';
  });
	
  test("Gets the param array correctly", function () {
    var route = '/products?names=apple&names=nectarine';
    Backbone.history.navigate(route, {trigger: true});
		var names = Backbone.history.params['names'];
    equal(names[0], 'apple');
		equal(names[1], 'nectarine');
		window.location.hash = '';
  });

	test("Builds the path with parameters", function () {
		var pathTo = Backbone.history.pathWith('/products/search', { name: 'apple', page: 1 }),
			expected = '/products/search?name=apple&page=1';
		equal(pathTo, expected);		
	})
  
	test("Builds the path with parameters array", function () {
		var pathTo = Backbone.history.pathWith('/products/search', { names: ['apple', 'nectarine'], page: 1 }),
			expected = '/products/search?names=apple&names=nectarine&page=1';
		equal(pathTo, expected);		
	})
